...
def calc_vwap(orders_depth):

    buy_price = np.array([item for item in orders_depth.buy_orders.keys() if not np.isnan(item)])

    buy_volume = np.array([item for item in orders_depth.buy_orders.values() if not np.isnan(item)])

    sell_price = np.array([item for item in orders_depth.sell_orders.keys() if not np.isnan(item)])

    sell_volume = -1 * np.array([item for item in orders_depth.sell_orders.values() if not np.isnan(item)])
    ...
    def calc_round1_order(self, product: str, order_depth: OrderDepth, 
        ...
        if product == 'AMETHYSTS':
            ...
            for ask, vol in order_sell.items():
                if ((ask < fair_bid) or ((self.position[product] < 0) and (ask == fair_ask))) and curr_pos < self.POSITION_LIMIT['AMETHYSTS']:
                    max_for_buy = max(max_for_buy, ask)
                    order_for = min(-vol, self.POSITION_LIMIT['AMETHYSTS'])
                    curr_pos += order_for
            ...
            for bid, vol in order_buy.items():
                if ((bid > fair_ask) or ((self.position[product]>0) and (bid == fair_ask))) and curr_pos > -self.POSITION_LIMIT['AMETHYSTS']:
